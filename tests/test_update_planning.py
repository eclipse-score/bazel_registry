...
class TestUpdatePlanning:
    """Test planning module updates."""

    def test_plan_finds_module_needing_update(
        self,
        make_module_info: Callable[..., BazelModuleInfo],
        make_release_info: Callable[..., GitHubReleaseInfo],
    ) -> None:
        ...
        gh.try_get_module_file_content.return_value = (
            f'module(version="{new_release_version}", compatibility_level={expected_comp_level})'
        )
        ...
        assert plan[0].release.version == new_release_version
